...
- **Skills & MCPs section** — two side-by-side tables (top 10 each) showing which skills and MCP servers were invoked across this app's sessions
...
| Skills (top 10) | `getAppSkills()` | `raw_session_skills` × `dim_sessions` |
| MCP servers (top 10) | `getAppMcps()` | `raw_session_mcps` × `dim_sessions` |
...
- **Skills & MCPs tables** — shows which plugins (skills/MCP servers) the app's sessions invoked; top 10 each by session count; "Last used" is the most recent session date in the range.
...
- **App with no skills/MCPs loaded** → "No skills loaded in this range." / "No MCP servers loaded in this range."
...
- **Range filter with no data** → KPIs fall back to lifetime; range-specific metrics (agents, people, skills, MCPs) may be empty or show cumulative zeros.

...
- **Ingest coverage (X)** — per-file ingestion coverage from the watcher artifact: files fully ingested vs. partial/unprocessed, bytes remaining, dropped lines, parse errors, and a scrollable table of remaining work
...
| Ingest coverage | /data/artifacts/ingest_coverage/latest.json |

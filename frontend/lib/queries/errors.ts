...
export async function getErrorsFiltered(kind?: string, since?: string | null) {
  ...
  // Use andTsFilter so that `since` is validated by assertTs before interpolation,
  // consistent with how tsFilter/andTsFilter guard every other query in this file.
  const tsClause = andTsFilter('e.ts', since ?? null) || `AND e.ts >= NOW() - INTERVAL '14 days'`

...
interface NumberIntegrity {
  generated_at: string
  total: number
  pass: number
  fail: number
  failing: { check: string; page: string; expected_source: string; actual: string }[]
}

interface IngestCoverageFile {
  path: string
  size: number
  offset: number
  status: 'partial' | 'unprocessed'
  bytes_remaining: number
  last_seen_at: string | null
  lines_total: number | null
  events_kept: number | null
  dropped_known: number | null
  dropped_unknown: number | null
  parse_errors: number | null
  last_error: string | null
}

interface IngestCoverage {
  generated_at: string
  scan_dirs: string[]
  summary: {
    files_total: number
    ...
    bytes_remaining: number
    events_kept: number
    lines_dropped_known: number
    lines_dropped_unknown: number
    parse_errors: number
  }
  files: IngestCoverageFile[]
}
...
interface Snapshot {
  ...
  errors: WatcherError[]
  integrity: NumberIntegrity | null
...
function deriveVerdict(d: Snapshot): Verdict {
  ...
  if (d.integrity && d.integrity.fail > 0) {
    issues.push({
      kind: 'warn',
      label: 'number integrity',
      body: `${d.integrity.fail} number-map check(s) failing`,
      jump: 'integrity',
    })
  }
...
function Crumb({ tone, refreshedAgo, refreshSec, isError }: { tone: Tone; refreshedAgo: number; refreshSec: number; isError: boolean }) {
  return (
    <div className="obs-crumb">
      ...
      <div className="obs-crumb-right" suppressHydrationWarning>
        {isError ? (
          <>
            <span style={{ color: TONE_HEX.bad }}>stale · poll failed</span>
            <span className="obs-crumb-sep">·</span>
            <span className={`obs-live-pill ${toneClass('bad')}`}>
              <Dot tone="bad" size={5} pulse />
              <span>DEGRADED · {refreshSec}s</span>
            </span>
          </>
        ) : (
          <>
...
function NumberIntegritySection({ integrity }: { integrity: NumberIntegrity | null }) {
  if (integrity === null) {
    return (
      <div className="obs-errors-empty">
        <span>No number_verify.json yet — the verify runner writes it after the next dbt cycle.</span>
      </div>
    )
  }
  if (integrity.fail === 0) {
    return (
      <div className="obs-errors-empty">
        ...
        <span>All {integrity.total} number-map checks <span style={{ color: TONE_HEX.good }}>passing</span>.</span>
      </div>
    )
  }
  return (
    <div className="obs-table-wrap">
      <table className="obs-table">
        ...
        <tbody>
          {integrity.failing.map((c, i) => (
            <tr key={i} className="is-error">
              <td><span style={{ fontWeight: 500 }}>{c.check}</span></td>
              <td>{c.page}</td>
              <td style={{ color: 'var(--muted)' }}>{c.expected_source}</td>
              <td style={{ color: TONE_HEX.bad }}>{c.actual}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function KpiGrid({ snapshot, verdict }: { snapshot: Snapshot; verdict: Verdict }) {
  ...
  const dbt = snapshot.dbt
  const overall = snapshot.overall
  // Bronze freshness must always come from overall.bronze_age_seconds — the
  // single canonical `NOW() - MAX(ts)` scan over raw_events computed in
  // getOverallHealth(). dbt's source_freshness entries are a *frozen*
  // snapshot recorded at the last `dbt source freshness` invocation and can
  // be stale by however long it's been since dbt last ran; using it here
  // previously caused this card to disagree with the Flow strip and the
  // medallion Bronze layer, which both derive from the live value.
  const ageStr = overall?.bronze_age_seconds != null ? fmtAgeShort(overall.bronze_age_seconds) : '—'
  ...
  const integrityTone: Tone = snapshot.integrity == null ? 'mute' : snapshot.integrity.fail === 0 ? 'good' : 'warn'
  ...
  return (
    <section className="obs-kpis">
      ...
      <div className="obs-kpi-col">
        ...
        <div className="obs-kpi">
          ...
          <div className="obs-kpi-sub">
            {overall?.bronze_age_seconds != null && bronzeTone === 'bad'
              ? `+${fmtAgeShort(overall.bronze_age_seconds - 1800)} past error`
              : 'within thresholds'}
          </div>
        </div>
        <div className="obs-kpi">
      ...
      <div className="obs-kpi-col">
        ...
        <div className="obs-kpi">
          ...
          <div className="obs-kpi-value">
            <span style={{ color: testsFail === 0 ? TONE_HEX.good : TONE_HEX.bad }}>{dbt?.tests_pass ?? 0}</span>
            <span className="obs-kpi-divider">/</span>
      ...
      <div className="obs-kpi-col">
        <div className="obs-kpi-head">
          <span className="obs-kpi-head-label">INTEGRITY</span>
          <Pill tone={integrityTone}>
            {snapshot.integrity == null ? 'PENDING' : snapshot.integrity.fail === 0 ? 'CLEAN' : `${snapshot.integrity.fail} FAILING`}
          </Pill>
        </div>
        <div className="obs-kpi">
          <div className="obs-kpi-label">checks</div>
          <div className="obs-kpi-value">
            <span style={{ color: integrityTone === 'good' ? TONE_HEX.good : integrityTone === 'warn' ? TONE_HEX.warn : 'var(--ink-2)' }}>
              {snapshot.integrity?.pass ?? '—'}
            </span>
            <span className="obs-kpi-divider">/</span>
            <span>{snapshot.integrity?.total ?? '—'}</span>
          </div>
          <div className="obs-kpi-sub">{snapshot.integrity ? (snapshot.integrity.fail === 0 ? 'all passing' : `${snapshot.integrity.fail} failing`) : 'verify not run yet'}</div>
        </div>
        <div className="obs-kpi">
          <div className="obs-kpi-label">generated</div>
          <div className="obs-kpi-value" suppressHydrationWarning>
            {snapshot.integrity ? fmtRelativeMinutes(snapshot.integrity.generated_at) : '—'}
          </div>
          <div className="obs-kpi-sub" suppressHydrationWarning>
            {snapshot.integrity ? fmtClock(snapshot.integrity.generated_at) : 'after next dbt cycle'}
          </div>
        </div>
      </div>
    </section>
...
// ---------------------------------------------------------------------------
// Ingest coverage panel
// ---------------------------------------------------------------------------

function statusTone(status: 'partial' | 'unprocessed'): Tone {
  return status === 'partial' ? 'warn' : 'mute'
}

function IngestCoveragePanel({ coverage }: { coverage: IngestCoverage | null }) {
  const s = coverage?.summary

  // Empty state: no artifact yet, or all files fully ingested (no remaining files)
  if (coverage === null) {
    return (
      <div className="obs-errors-empty">
        <span>No ingest coverage snapshot yet — the watcher writes /data/artifacts/ingest_coverage/latest.json after the first scan.</span>
      </div>
    )
  }

  const allDone = coverage.files.length === 0

  return (
    <section style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      {/* Header verdict */}
      <div className="obs-layer-head" style={{ gap: 12 }}>
        {allDone ? (
          <>
            <Dot tone="good" size={8} />
            <span style={{ color: TONE_HEX.good, fontWeight: 500 }}>
              All {s?.files_full ?? s?.files_total ?? 0} of {s?.files_total ?? 0} files fully ingested
            </span>
          </>
        ) : (
          <>
            <Dot tone="warn" size={8} pulse />
            <span style={{ fontWeight: 500 }}>
              <span style={{ color: TONE_HEX.good }}>{s?.files_full ?? 0}</span>
              {' of '}
              <span>{s?.files_total ?? 0}</span>
              {' files fully ingested · '}
              <span style={{ color: TONE_HEX.warn }}>
                {(s?.files_partial ?? 0) + (s?.files_unprocessed ?? 0)} remaining
              </span>
            </span>
          </>
        )}
        {coverage.generated_at && (
          <span className="obs-rule-sub" style={{ marginLeft: 'auto' }} suppressHydrationWarning>
            snapshot {fmtRelativeMinutes(coverage.generated_at)}
          </span>
        )}
      </div>

      {/* Summary stats grid */}
      <div className="obs-kpis" style={{ gap: 0, borderTop: '1px dashed var(--rule)', paddingTop: 18 }}>
        <div className="obs-kpi-col">
          <div className="obs-kpi">
            <div className="obs-kpi-label">files · full</div>
            <div className="obs-kpi-value" style={{ color: TONE_HEX.good }}>{fmtNum(s?.files_full ?? 0)}</div>
            <div className="obs-kpi-sub">of {fmtNum(s?.files_total ?? 0)} total</div>
          </div>
          <div className="obs-kpi">
            <div className="obs-kpi-label">files · partial</div>
            <div className="obs-kpi-value" style={(s?.files_partial ?? 0) > 0 ? { color: TONE_HEX.warn } : undefined}>
              {fmtNum(s?.files_partial ?? 0)}
            </div>
          </div>
          <div className="obs-kpi">
            <div className="obs-kpi-label">files · unprocessed</div>
            <div className="obs-kpi-value" style={(s?.files_unprocessed ?? 0) > 0 ? { color: TONE_HEX.warn } : undefined}>
              {fmtNum(s?.files_unprocessed ?? 0)}
            </div>
          </div>
        </div>
        ...
        <div className="obs-kpi-col">
          <div className="obs-kpi">
            <div className="obs-kpi-label">bytes · remaining</div>
            <div className="obs-kpi-value" style={(s?.bytes_remaining ?? 0) > 0 ? { color: TONE_HEX.warn } : undefined}>
              {fmtBytes(s?.bytes_remaining ?? 0)}
            </div>
            <div className="obs-kpi-sub">{fmtBytes(s?.bytes_ingested ?? 0)} ingested of {fmtBytes(s?.bytes_total ?? 0)}</div>
          </div>
          <div className="obs-kpi">
            <div className="obs-kpi-label">events kept</div>
            <div className="obs-kpi-value">{fmtNum(s?.events_kept ?? 0)}</div>
          </div>
        </div>

        <div className="obs-kpi-col">
          <div className="obs-kpi">
            <div className="obs-kpi-label">dropped · known</div>
            <div className="obs-kpi-value">{fmtNum(s?.lines_dropped_known ?? 0)}</div>
            <div className="obs-kpi-sub">expected event types</div>
          </div>
          <div className="obs-kpi">
            <div className="obs-kpi-label">dropped · unknown</div>
            <div className="obs-kpi-value" style={(s?.lines_dropped_unknown ?? 0) > 0 ? { color: TONE_HEX.warn } : undefined}>
              {fmtNum(s?.lines_dropped_unknown ?? 0)}
            </div>
            <div className="obs-kpi-sub">unexpected event types</div>
          </div>
          <div className="obs-kpi">
            <div className="obs-kpi-label">parse errors</div>
            <div className="obs-kpi-value" style={(s?.parse_errors ?? 0) > 0 ? { color: TONE_HEX.bad } : undefined}>
              {fmtNum(s?.parse_errors ?? 0)}
            </div>
          </div>
        </div>

        {(coverage.scan_dirs?.length ?? 0) > 0 && (
          <div className="obs-kpi-col">
            <div className="obs-kpi">
              <div className="obs-kpi-label">scan dirs</div>
              <div className="obs-kpi-value" style={{ fontSize: 13 }}>{coverage.scan_dirs.length}</div>
              <div className="obs-kpi-sub" style={{ wordBreak: 'break-all' }}>
                {coverage.scan_dirs.join(', ')}
              </div>
            </div>
          </div>
        )}
      ...
      {allDone ? (
        <div className="obs-errors-empty">
          <Dot tone="good" size={8} />
          <span>All discovered files fully ingested — nothing remaining.</span>
        </div>
      ) : (
        <div className="obs-table-wrap" style={{ maxHeight: 360, overflowY: 'auto' }}>
          <table className="obs-table">
            <thead>
              <tr>
                <th>status</th>
            ...
            <tbody>
              {coverage.files.map((f, i) => {
                ...
                return (
                  <tr key={i} className={f.status === 'unprocessed' ? '' : 'is-warn'}>
                    <td>
                      <Pill tone={tone}>{f.status}</Pill>
                    </td>
                    <td>
                      <span className="obs-td-strong" title={f.path}>{basename}</span>
                      <span className="obs-td-sub" style={{ color: 'var(--muted)', display: 'block', fontSize: 10, maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{dir}</span>
                    </td>
                    <td>{fmtBytes(f.size)}</td>
                    <td style={{ color: f.bytes_remaining > 0 ? TONE_HEX.warn : undefined }}>
                      {fmtBytes(f.bytes_remaining)}
                    </td>
                    <td>{f.dropped_known != null ? fmtNum(f.dropped_known) : '—'}</td>
                    <td style={{ color: (f.dropped_unknown ?? 0) > 0 ? TONE_HEX.warn : undefined }}>
                      {f.dropped_unknown != null ? fmtNum(f.dropped_unknown) : '—'}
                    </td>
                    <td style={{ color: (f.parse_errors ?? 0) > 0 ? TONE_HEX.bad : undefined }}>
                      {f.parse_errors != null ? fmtNum(f.parse_errors) : '—'}
                    </td>
...
export function PipelineLive({ initialData }: { initialData: Snapshot | null }) {
  ...
  useEffect(() => {
    function poll() {
      fetch('/api/observability', { cache: 'no-store' })
        ...
        .catch(() => {
          // keep stale data — flag the error so the Crumb shows a degraded indicator
          setIsError(true)
        })
    }
    if (!initialData) poll()
  ...
  return (
    <div>
      <Crumb tone={verdict.state} refreshedAgo={secondsAgo} refreshSec={REFRESH_SEC} isError={isError} />
      ...
      <RuleHeader
        id="integrity"
        label="IX · Number integrity"
        sub={data.integrity
          ? `${data.integrity.pass}/${data.integrity.total} checks passing · last run ${fmtRelativeMinutes(data.integrity.generated_at)}`
          : 'verify runner not yet invoked'}
      />
      <NumberIntegritySection integrity={data.integrity} />

      <RuleHeader
        id="coverage"
        label="X · Ingest coverage"
        sub={data.coverage
          ? `${data.coverage.summary.files_full}/${data.coverage.summary.files_total} files fully ingested · ${data.coverage.files.length} remaining`
          : '/data/artifacts/ingest_coverage/latest.json'}
      />
      <IngestCoveragePanel coverage={data.coverage} />

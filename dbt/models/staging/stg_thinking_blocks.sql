...
SELECT
    ...
    ROW_NUMBER() OVER (
        ...
        -- byte_offset as tiebreaker: multiple thinking blocks can share the
        -- same ts within one assistant event (D-L8). byte_offset reflects
        -- physical order in the JSONL file, giving a stable, deterministic
        -- index even when timestamps collide.
        ORDER BY ts, byte_offset
    ) - 1                                                   AS thinking_idx,

...
- **Eyebrow chips:** Provider, **SDK badge** (only when the session came from an Agent SDK trace, `source='sdk_trace'`), **run-outcome status pill** (`session_status`: completed / budget-killed / interrupted / error / unknown), person, cwd (folder name only), agent strip (all agents + "(N agents)" tally if > 1), git branch, 🧩 N skills, ⚡ N MCPs
...
- **Right:** SESSION COST hero stat with footer: X prompts · Y turns · Z tokens · N files. For SDK-trace sessions with a known `max_turns`, a **budget gauge** (`turns_used / max_turns`, filled to `budget_utilization`) renders below the cost; Claude sessions show none

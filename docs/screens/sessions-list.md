...
- **Table columns:** Started · Status · Person · App · Agent · Title · Model · Turns · Commits · 🧩 · ⚡ · Cost · [arrow]
  - **Started:** Date and local time; duration if completed
  - **Status:** Run-outcome pill from `dim_sessions.session_status` (completed / budget-killed / interrupted / error / unknown); budget-kills and errors are colour-flagged, `completed` is quiet
  ...
  - **SDK badge:** Sessions ingested from an Agent SDK trace (`source='sdk_trace'`) show a small `SDK` pill beside the model; interactive Claude Code sessions show none
